  ...
  for(int i = 0 ; i < A->nrow; i ++) {
    ...
    for(int j = 0 ; j < B->ncol; j ++) {
      ...
      for(int k = startColIdx ; k < endColIdx ; k++) {
        sum += A->val[k] * B->val[A->col[k] * B->ncol + j];
      }
      C->val[i * B->ncol + j] = sum;
    }
  }
}

__global__
void multiply_one(int* _Arow, int* _Acol, float* _Aval, float* _B, float* _C,
                  int _ARowSize, int _BColSize, int _CRowSize, int _CColSize) {
  int tx = threadIdx.x; int ty = threadIdx.y;
  int bx = blockIdx.x; int by = blockIdx.y;
  int bdx = blockDim.x; int bdy = blockDim.y;
  
  int targetRow = bdy * by + ty;
  int targetCol = bdx * bx + tx;
  
  if(targetRow < _CRowSize && targetCol < _CColSize) {
    int startColIdx = _Arow[targetRow];
    int endColIdx = targetRow + 1 < _ARowSize ? _Arow[targetRow + 1] : _ARowSize;
    double sum = 0;
    for(int k = startColIdx ; k < endColIdx ; k++)
      sum += _Aval[k] * _B[_Acol[k] * _BColSize + targetCol];
    _C[targetRow * _CColSize + targetCol] = sum;
  }
}
...
__global__
void print_device_vector(float* _b, int _size) {
  printf("--------------------------------------------------\n");
  for(int i = 0 ; i < _size ; i ++)
    printf("%5.2f", _b[i]);
  printf("\n");
...
void multiply_cuda(struct sparse_mtx *A, struct dense_mtx *B, struct dense_mtx *C) {
  ...
  cudaMalloc(&dA_row, (A->nrow) * sizeof(int));
  cudaMalloc(&dA_col, (A->nnze) * sizeof(int));
  cudaMalloc(&dA_val, (A->nnze) * sizeof(float));
  cudaMalloc(&dB, (B->nrow) * (B->ncol) * sizeof(float));
  cudaMalloc(&dC, (C->nrow) * (C->ncol) * sizeof(float));
  cudaMemcpy(dA_row, A->row, sizeof(int) * (A->nrow), cudaMemcpyHostToDevice);
  cudaMemcpy(dA_col, A->col, sizeof(int) * (A->nnze), cudaMemcpyHostToDevice);
  cudaMemcpy(dA_val, A->val, sizeof(float) * (A->nnze), cudaMemcpyHostToDevice);
  cudaMemcpy(dB, B->val, sizeof(float) * (B->nrow) * (B->ncol), cudaMemcpyHostToDevice);

  /*execute kernel*/
  dim3 grid( (C->ncol + BLOCK_WIDTH - 1)/BLOCK_WIDTH, (C->nrow + BLOCK_WIDTH - 1) / BLOCK_WIDTH, 1);
  dim3 block(BLOCK_WIDTH, BLOCK_WIDTH, 1);
  multiply_one<<<grid, block>>>(dA_row, dA_col, dA_val, dB, dC, A->nrow, B->ncol, C->nrow, C->ncol);
  cudaDeviceSynchronize();

  cudaMemcpy(C->val, dC, sizeof(float) * (C->nrow) * (C->ncol), cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
